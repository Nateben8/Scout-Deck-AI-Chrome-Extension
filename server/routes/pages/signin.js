export default async function signinPage(req, res) {
...
<body>
  <div class="card">
    <div class="head"><div style="font-weight:800">Sign in to ScoutDeck</div></div>
    <div class="content">
      <form id="form">
        <label>Email</label>
      ...
      <div class="muted">New here? <a class="link" href="/signup${redirect ? `?redirect=${encodeURIComponent(redirect)}` : ''}">Create an account</a></div>
    </div>
  </div>
  <script>
    ...
    form.addEventListener('submit', async (e)=>{
      ...
      try{
        ...
        // If redirect provided (extension auth), redirect with tokens in hash
        if(redirect){
          ...
          // Web flow: store tokens and go to dashboard
